...
class UpdateHourlyFinishingProductionDataInTotalProductionReport extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'update:hour-wise-finishing-data-in-total-production-report';
     ...
     * @var string
     */
    protected $description = 'Update Hourly Finishing Production Data In TotalProductionReport Model';
                ...
                ->chunk($chunk_data_count, function ($finishingProductions) use (&$counter) {
                    foreach ($finishingProductions as $finishingProduction) {
                        $exception = DB::transaction(function () use ($finishingProduction) {
                            $totalColumn = self::TOTAL_PRODUCTION_REPORT_COLUMN_MAPPING[$finishingProduction->production_type];
