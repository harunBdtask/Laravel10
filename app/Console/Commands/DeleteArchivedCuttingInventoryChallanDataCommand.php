...
class DeleteArchivedCuttingInventoryChallanDataCommand extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'delete:archived-cutting-inventory-challan-data';
     ...
     * @var string
     */
    protected $description = 'Delete Archived CuttingInventoryChallan Data Command';
    ...
    public function handle()
    {
        try {
            ...
            DB::table('archived_cutting_inventory_challans')
                ->select('challan_no')
                ...
                ->chunk($chunk_data_count, function ($data_details) use (&$counter) {
                    ...
                    if (count($deleteable_challans)) {
                        DB::table('cutting_inventory_challans')->whereIn('challan_no', $deleteable_challans)->delete();
                    }
    ...
    private function getDeletableChallans($data_details) : array
    {
        ...
        foreach ($data_details as $detail) {
            if (!DB::table('cutting_inventories')->where('challan_no', $detail->challan_no)->count()) {
